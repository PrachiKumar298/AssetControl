...
export default function PPF() {
  ...
  const [editStates, setEditStates] = useState({}); // { profileId: { name, ppf_invested, ppf_current } }
  ...
  const handleSaveProfile = async (profileId) => {
    ...
    const ppf_invested = parseFloat(state.ppf_invested);
    const ppf_current = parseFloat(state.ppf_current);
    if (isNaN(ppf_invested) || ppf_invested < 0 || isNaN(ppf_current) || ppf_current < 0) {
  ...
  // Compounded values
  const totalInvested = profiles.reduce((sum, p) => sum + parseFloat(p.ppf_invested || 0), 0);
  const totalCurrent = profiles.reduce((sum, p) => sum + parseFloat(p.ppf_current || 0), 0);
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center space-x-3 border-b border-brand-border/60 pb-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-brand-dark">Public Provident Fund (PPF)</h2>
          <p className="text-sm text-brand-dark/50">Manage long-term, tax-saving PPF investments per profile</p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="text-center py-12 text-brand-dark/50">Loading PPF profiles...</div>
      ) : (
        <div className="space-y-8">
          {/* Main Profiles Table */}
          <div className="space-y-3">
            <h3 className="text-lg font-bold text-brand-dark px-1">Personal PPF Accounts</h3>
                  ...
                  {profiles.length === 0 ? (
                    <tr>
                      <td colSpan="5" className="table-row-cell text-center text-brand-dark/50 py-8">
                        No profiles created yet. Create a profile above to start managing PPF details.
                      </td>
                    </tr>
                  ) : (
                    profiles.map((p) => {
                      ...
                      const hasChanged = state.name !== p.name || 
                        parseFloat(state.ppf_invested) !== parseFloat(p.ppf_invested) ||
                        parseFloat(state.ppf_current) !== parseFloat(p.ppf_current);

                      const profit = parseFloat(state.ppf_current) - parseFloat(state.ppf_invested);
                          ...
                          <td className="table-row-cell">
                            <div className="flex items-center space-x-1">
                              ...
                              <input
                                type="number"
                                step="0.01"
                                value={state.ppf_invested}
                                onChange={(e) => handleInputChange(p.id, 'ppf_invested', e.target.value)}
                          ...
                          <td className="table-row-cell">
                            <div className="flex items-center space-x-1">
                              ...
                              <input
                                ...
                                onChange={(e) => handleInputChange(p.id, 'ppf_current', e.target.value)}
                                className="w-full bg-transparent border-0 border-b border-transparent focus:border-brand-blue rounded-none px-0 py-0.5 focus:ring-0 text-sm font-mono font-medium"
                              />
                            </div>
                          </td>
          ...
          <div className="space-y-3">
            <h3 className="text-lg font-bold text-brand-dark px-1">Compounded PPF Statement</h3>
            <div className="table-container">
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-brand-blue/5">
